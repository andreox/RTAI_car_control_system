...
static void * acquire_loop(void * par) {
	...
	if (!(read_Task = rt_task_init_schmod(nam2num("READE"), 1, 0, 0, SCHED_FIFO, CPUMAP))) {
	...
	while (keep_on_running)
	{
		...
		prev_sen=buffer[head];
		buffer[head] = (sensor[1]); //Velocità memorizzata in sensor viene messa nel buffer
		curr_buff= buffer[head];
		
		if( (prev_sen-curr_buff) >= 30) {
			ris=prev_sen-curr_buff;
			 rt_mbx_send_if(mbx1, &crash, sizeof(int)) ; rt_printk("Valore ruota 2: %d \n",ris);
			 }
		...
		diag[1].buff = curr_buff ;
		
		diag[1].exc_acquire = acq;
...
static void * filter_loop(void * par) {
	...
	if (!(filter_Task = rt_task_init_schmod(nam2num("FILTE"), 3, 0, 0, SCHED_FIFO, CPUMAP))) {
...
static void * control_loop(void * par) {
	...
	unsigned int ANTI_SKID_ON = 0;		//to activate the ANTI SKID
	...
	if (!(control_Task = rt_task_init_schmod(nam2num("CNTR"), 5, 0, 0, SCHED_FIFO, CPUMAP))) {
	...
	{    
		...
		if(!rt_mbx_receive_if(mbx, &block_if_other_wheel_is_blocked, sizeof(int)))
			rt_printk(" L'altra ruota sta slittando, ha attivato l'Anti-Skid, blocco anche questa (RUOTA 1) \n"); //****
		if (block_if_other_wheel_is_blocked==1) ANTI_SKID_ON = 1;
			

		// evaluating if the wheel is blocked
        if(prev_sensor==(sensor[1])) block = 1;
        else block = 0; 
		...
		if (ANTI_SKID_ON) {
		 //se ho attivato l'anti slittamento
			if (((*reference)==0) && (plant_state!=0) && (block!=1)) {
				...
				int blocca = 1 ; //***
				rt_printk("INVIO MESSAGGIO SONO IL CONTROLLER 1 %d\n",blocca);
...
static void * actuator_loop(void * par) {
	...
	if (!(write_Task = rt_task_init_schmod(nam2num("WRIT"), 7, 0, 0, SCHED_FIFO, CPUMAP))) {
...
int main(void)
{
	...
	if (!(main_Task = rt_task_init_schmod(nam2num("MANTSK"), 0, 0, 0, SCHED_FIFO, 0xF))) {
	...
	space_avail = rt_typed_sem_init(SPACE_SEM1, BUF_SIZE, CNT_SEM | PRIO_Q);
	meas_avail = rt_typed_sem_init(MEAS_SEM1, 0, CNT_SEM | PRIO_Q);
	...
	while (keep_on_running) {
		printf("Control1: %d %d",(*actuator), block_if_other_wheel_is_blocked);
		printf("\n");
		rt_sleep(10000000);
	}

	rt_shm_free(SEN_SHM1);
	rt_shm_free(ACT_SHM1);
	rt_shm_free(REFSENS);
